...
public class GetListofIssues {
	...
	public void jiraDeleteIssue() {
		...
		Response Resp = given().header("Content-Type", "application/json").header("Cookie", "JSESSIONID=" + sessionid)
				.body("{\r\n" + "	\"fields\": {\r\n" + "        \"project\":\r\n" + "        {\r\n"
						+ "            \"key\": \"RES\"\r\n" + "        },\r\n" + "        	\r\n"
						+ "        \"summary\": \"Error4 while Running Regression \",\r\n"
		...
		// "JSESSIONID=" + sessionid).when()
		// .delete("/rest/api/2/issue/" + id).then().assertThat().statusCode(204);

		// Get All issues

		Response list = given().when().get("/rest/api/2/issuetype").then().assertThat().statusCode(200).extract()
				.response();
		JsonPath js2 = new JsonPath(list.asString());
		js2.get();
		System.out.println("**** " + js2.get());
